...
import { View, Text, Image, ScrollView, Alert, Modal, ActivityIndicator } from "react-native";
...
export default function ItemDetail() {
  ...
  const handleLiked = () => {
    ...
    try {
      axios.post(`${hostId}:80/api/addLiked`, post)
        .then((res) => {
  ...
  return (
    <View className="h-full bg-white">
      ...
      <Ionicons
        ...
        size={30} className=" absolute right-2 top-5 z-10"
        onPress={() => handleLiked()}
        color="#0B9A61" />
